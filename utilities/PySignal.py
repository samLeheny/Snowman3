...
ALL_SIGNALS = weakref.WeakSet()
ALL_CLASS_SIGNALS = weakref.WeakSet()
    ...
    def emit(self, *args, **kwargs):
        ...
        for slot in self._slots:
            ...
            if not slot:
                ...
                if DEBUG:
                    logging.getLogger('rig_build').info('PySignal slot %s --> %s' % slot)
                slot()
                ...
                for obj, method in slot.items():
                    if DEBUG:
                        logging.getLogger('rig_build').info(f'PySignal method --> {method} {args}, {kwargs}')
                ...
                if slot() is not None:
                    if DEBUG:
                        logging.getLogger('rig_build').info(f'PySignal weakref --> {slot()} {args}, {kwargs}')
                    slot()(*args, **kwargs)
            else:
