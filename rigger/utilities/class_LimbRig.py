...
class LimbRig:
    ...
    def create_pin_ctrl(self, name, target_jnt_index, limb_span_jnt_indices):
        ...
        buffer_node = gen.buffer_obj(ctrl, parent_=pos_offset_node)
    ...
    def create_blend_jnts(self):
        ...
        self.blend_jnt_chain_buffer = gen.buffer_obj(self.segments[0].blend_jnt)
        if self.side == nom.rightSideTag:
    ...
    def create_ik_jnt_chain(self):
        ...
        #...Wrap top of chain in a buffer group -----------------------------------------------------------------------
        self.ik_jnt_chain_buffer = gen.buffer_obj(self.segments[0].ik_jnt, parent_=self.ctrls['socket'])
    ...
    def create_ik_ctrls(self, tarsus_index=None):
        ...
        extrem_buffer = gen.buffer_obj(self.ctrls['ik_extrem'], parent_=self.grps['transform'])
        ...
        self.ctrls['ik_pv'] = rig.control(ctrl_info = {'shape': 'sphere',
                                          ...
                                          side = self.side,
                                          color = self.ctrl_colors["IK"])
        pv_buffer = gen.buffer_obj(self.ctrls['ik_pv'], parent_=self.grps['transform'])
        ...
        if tarsus_index:
            self.ctrls['ik_tarsus'] = rig.control(ctrl_info={
                ...
                side=self.side,
                color=self.ctrl_colors['IK'])
            tarsus_buffer = gen.buffer_obj(self.ctrls['ik_tarsus'], parent_=self.ctrls['ik_extrem'])
