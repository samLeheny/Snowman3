...
class BlueprintIO:
    ...
    ####################################################################################################################
    def save(self, dirpath, blueprint):
        blueprint_data = self.data_from_blueprint(blueprint)
    ...
    ####################################################################################################################
    def data_from_blueprint(self, blueprint):
        data = {
    ...
    def load(self, dirpath):
        blueprint_data = self.data_from_file(dirpath)
        blueprint = self.blueprint_from_data(blueprint_data)
        return blueprint
    ...
    def update_blueprint_from_scene(self, blueprint):
        ...
        blueprint = self.blueprint_from_data(blueprint_data)
        self.save(dirpath=f"{blueprint_data['dirpath']}/working", blueprint=blueprint)
    ...
    ####################################################################################################################
    def add_part_to_module(self, part, module):
        ...
        self.update_module_in_blueprint(module)
        return scene_part


    ####################################################################################################################
    def remove_module_from_blueprint(self, module):
        dirpath = r'C:\Users\61451\Desktop\test_build\working'
        blueprint_IO = BlueprintIO()
        working_blueprint = blueprint_IO.blueprint_from_file(dirpath)
        working_blueprint.modules.pop(f'{module.side_tag}{module.name}')
        # ...Update disk
        blueprint_IO = BlueprintIO()
        blueprint_IO.save(dirpath=dirpath, blueprint=working_blueprint)


    ####################################################################################################################
    def remove_scene_module(self, module):
        ...
        self.remove_module_from_blueprint(module)
        return True


    ####################################################################################################################
    def add_module_to_blueprint(self, module, check_for_clashes=True):
        dirpath = r'C:\Users\61451\Desktop\test_build\working'
        blueprint_IO = BlueprintIO()
        working_blueprint = blueprint_IO.blueprint_from_file(dirpath)
    ...
    ####################################################################################################################
    def update_module_in_blueprint(self, module):
        self.add_module_to_blueprint(module, check_for_clashes=False)


    ####################################################################################################################
    def add_part_to_blueprint(self, part, check_for_clashes=True):
        dirpath = r'C:\Users\61451\Desktop\test_build\working'
        blueprint_IO = BlueprintIO()
        working_blueprint = blueprint_IO.blueprint_from_file(dirpath)
    ...
    ####################################################################################################################
    def remove_part_from_blueprint(self, part):
        dirpath = r'C:\Users\61451\Desktop\test_build\working'
        blueprint_IO = BlueprintIO()
        working_blueprint = blueprint_IO.blueprint_from_file(dirpath)
